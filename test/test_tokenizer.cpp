...
TEST_CASE("Test Single Tokens") {
    SECTION("Test Directive") {
        ...
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::DIRECTIVE, "data"}};
        REQUIRE(expectedTokens == actualTokens);
    }
    SECTION("Test Label Declaration") {
        const std::string line = "label:";
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::LABEL, "label"}};
        REQUIRE(expectedTokens == actualTokens);
    }
    SECTION("Test Label Reference") {
        const std::string line = "j label";
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::INSTRUCTION, "j"},
    ...
    SECTION("Test Instruction") {
        ...
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::INSTRUCTION, "addi"}};
        REQUIRE(expectedTokens == actualTokens);
    }
    SECTION("Test Register") {
        const std::string line = "$v0";
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::REGISTER, "v0"}};
        REQUIRE(expectedTokens == actualTokens);
    }
    SECTION("Test Immediate") {
        std::string line = "42";
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::IMMEDIATE, "42"}};
        REQUIRE(expectedTokens == actualTokens);
    ...
    SECTION("Test Seperator") {
        ...
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::SEPERATOR, ","}};
        REQUIRE(expectedTokens == actualTokens);
    }
    SECTION("Test String") {
        const std::string line = R"("'ello \n\"There\"")";
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::STRING, R"('ello \n\"There\")"}};
        REQUIRE(expectedTokens == actualTokens);
    }
}
...
TEST_CASE("Test Lines") {
    SECTION("Test Labeled String") {
        ...
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::LABEL, "out_string"},
                                             {TokenType::DIRECTIVE, "asciiz"},
                                             {TokenType::STRING, R"(\nHello, World!\n)"}};
        REQUIRE(expectedTokens == actualTokens);
    }
    SECTION("Test Immediate Pseudo-Instruction") {
        ...
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::INSTRUCTION, "li"},
    ...
    SECTION("Test I-Type Instruction") {
        ...
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::INSTRUCTION, "lw"},
                                             {TokenType::REGISTER, "v0"},
                                             {TokenType::SEPERATOR, ","},
    ...
    SECTION("Test R-Type Instruction") {
        ...
        std::vector<Token> expectedTokens = {
                {TokenType::INSTRUCTION, "add"}, {TokenType::REGISTER, "t0"},
                {TokenType::SEPERATOR, ","},     {TokenType::REGISTER, "t1"},
                {TokenType::SEPERATOR, ","},     {TokenType::REGISTER, "t2"}};
        REQUIRE(expectedTokens == actualTokens);
    }
    SECTION("Test J-Type Instruction") {
        ...
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::INSTRUCTION, "j"},
                                             {TokenType::IMMEDIATE, "2048"}};
        REQUIRE(expectedTokens == actualTokens);
    }
    SECTION("Test Syscall") {
        const std::string line = R"(syscall)";
        std::vector<Token> actualTokens = Tokenizer::tokenizeLine(line);
        std::vector<Token> expectedTokens = {{TokenType::INSTRUCTION, "syscall"}};
        REQUIRE(expectedTokens == actualTokens);
    }
