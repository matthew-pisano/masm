...
TEST_CASE("Test add Instruction") {
    ...
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "add"},
    ...
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x2a, 0x40, 0x20});
...
TEST_CASE("Test addu Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "addu"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x2a, 0x40, 0x21});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test addi Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "addi"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x21, 0x28, 0xff, 0xfe});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test addiu Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "addiu"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x25, 0x28, 0xff, 0xfe});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test and Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "and"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x2a, 0x40, 0x24});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test andi Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "andi"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x31, 0x28, 0x00, 0xff});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test div Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "div"},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t2"}}};
        REQUIRE_NOTHROW(validateTokenLines(expectedTokens, actualTokens));
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x2a, 0x00, 0x1a});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test divu Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "divu"},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t2"}}};
        REQUIRE_NOTHROW(validateTokenLines(expectedTokens, actualTokens));
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x2a, 0x00, 0x1b});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test mfhi Instruction") {
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x00, 0x00, 0x40, 0x10});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test mflo Instruction") {
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x00, 0x00, 0x40, 0x12});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test mthi Instruction") {
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x20, 0x00, 0x11});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test mtlo Instruction") {
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x20, 0x00, 0x13});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test mult Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "mult"},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t2"}}};
        REQUIRE_NOTHROW(validateTokenLines(expectedTokens, actualTokens));
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x2a, 0x00, 0x18});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test multu Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "multu"},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t2"}}};
        REQUIRE_NOTHROW(validateTokenLines(expectedTokens, actualTokens));
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x2a, 0x00, 0x19});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test nor Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "nor"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x2a, 0x40, 0x27});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test or Instruction") {
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x2a, 0x40, 0x25});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test ori Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "ori"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x35, 0x28, 0x00, 0xff});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test sll Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "sll"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x00, 0x09, 0x41, 0x00});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test srl Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "srl"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x00, 0x09, 0x41, 0x02});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test sra Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "sra"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x00, 0x09, 0x41, 0x03});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test sllv Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "sllv"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x49, 0x40, 0x04});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test srlv Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "srlv"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x49, 0x40, 0x06});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test srav Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "srav"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x49, 0x40, 0x07});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test sub Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "sub"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x2a, 0x40, 0x22});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test subu Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "subu"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x2a, 0x40, 0x23});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test xor Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "xor"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x01, 0x2a, 0x40, 0x26});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
...
TEST_CASE("Test xori Instruction") {
    ...
    const std::vector<LineTokens> actualTokens = Tokenizer::tokenizeFile({rawFile});
    SECTION("Test Tokenize") {
        const std::vector<std::vector<Token>> expectedTokens = {{{TokenCategory::INSTRUCTION, "xori"},
                                                                 {TokenCategory::REGISTER, "t0"},
                                                                 {TokenCategory::SEPERATOR, ","},
                                                                 {TokenCategory::REGISTER, "t1"},
                                                                 {TokenCategory::SEPERATOR, ","},
    ...
    const MemLayout actualLayout = parser.parse(actualTokens, true);
    SECTION("Test Parse") {
        const std::vector<std::byte> expectedBytes = iV2bV({0x39, 0x28, 0xff, 0xff});
        const std::vector<std::byte> actualBytes = actualLayout.data.at(MemSection::TEXT);
        REQUIRE(expectedBytes == actualBytes);
    }
